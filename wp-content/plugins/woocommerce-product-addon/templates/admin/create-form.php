...
function render_input_settings($settings, $values = '') {
	...
	if ($settings != '') {
		...
		foreach ( $settings as $meta_type => $data ) {
			...
			// if( $data ['type'] == 'html-conditions' ) continue;
			...
			$setting_html	.= 'data-type="' . $data ['type'] . '" data-name="' . $meta_type . '">' . render_input_types ( $data ['type'], $meta_type, $data_values, $input_data_options, $placeholders) . '</td>';
 ...
 */
function render_input_types($type, $name, $value = '', $options = '', $placeholders='') {
	...
	switch ($type) {
		...
		case 'paired' :
			...
			$placeholder1 = (!empty($placeholders)) ? $placeholders[0] : 'Option';
			$placeholder2 = (!empty($placeholders)) ? $placeholders[1] : 'Price (optional)';
			...
			if($value){
				foreach ($value as $option){
					...
					$html_input .= '<input type="text" name="options[option]" value="'.$option['option'].'" placeholder="'.$placeholder1.'">';
					$html_input .= '<input type="text" name="options[price]" value="'.$option['price'].'" placeholder="'.$placeholder2.'">';
				...
				$html_input .= '<li class="data-options">';
				$html_input .= '<span class="dashicons dashicons-move"></span>';
				$html_input .= '<input type="text" name="options[option]" placeholder="'.$placeholder1.'">';
				$html_input .= '<input type="text" name="options[price]" placeholder="'.$placeholder2.'">';
				$html_input	.= '<img class="add_option" src="'.$plugin_meta['url'].'/images/plus.png" title="add rule" alt="add rule" style="cursor:pointer; margin:0 3px;">';
				$html_input	.= '<img class="remove_option" src="'.$plugin_meta['url'].'/images/minus.png" title="remove rule" alt="remove rule" style="cursor:pointer; margin:0 3px;">';
				$html_input .= '</li>';
			}
		...
		case 'paired-quantity' :
			...
			if($value){
				foreach ($value as $option){
					...
					$html_input .= '<input type="text" name="options[option]" value="'.$option['option'].'" placeholder="'.__('option',"ppom").'">';
					$html_input .= '<input type="text" name="options[price]" value="'.$option['price'].'" placeholder="'.__('price (if any)',"ppom").'">';
					$html_input .= '<input type="text" name="options[min]" value="'.$option['min'].'" placeholder="'.__('Min. Qty',"ppom").'">';
					$html_input .= '<input type="text" name="options[max]" value="'.$option['max'].'" placeholder="'.__('Max. Qty',"ppom").'">';
					$html_input	.= '<img class="add_option" src="'.$plugin_meta['url'].'/images/plus.png" title="add rule" alt="add rule" style="cursor:pointer; margin:0 3px;">';
					$html_input	.= '<img class="remove_option" src="'.$plugin_meta['url'].'/images/minus.png" title="remove rule" alt="remove rule" style="cursor:pointer; margin:0 3px;">';
					$html_input .= '</div>';
				}
