		...
		if( ppom_is_file_image($file_name) ) {
			...
			if(file_exists($thumb_dir_path)){
				...
				$response = array(
						...
						'nocache'			=> time(),
						'file_url'			=> ppom_get_dir_url() . $file_name,
						'html'				=> ppom_uploaded_file_preview($file_dir_path, $file_name, $is_image=true, $_REQUEST['settings']),
				);
			}else{
				$response = array(
			...
			$response = array(
					...
					'file_w'			=> 'na',
					'file_h'			=> 'na',
					'html'				=> ppom_uploaded_file_preview($file_dir_path, $file_name, $is_image=false, $_REQUEST['settings']),
			);
		}
	}
...
function ppom_uploaded_file_preview($file_dir_path, $file_name, $is_image, $settings){
	...
	$file_name = ppom_files_trim_name( $file_name );
	$html .= '<td class="nm-imagetools" style="padding-left: 5px; vertical-align:top"><h4>'.$file_name.'</h4><br>';
	$html .= '<span class="file-meta">'.$file_meta.'</span><br>';
	$html .= $file_tools;
	$html .= '</td>';
...
function ppom_files_trim_name( $file_name ) {
	...
	$max_chars = apply_filters('ppom_trim_file_maxchar', 16);
...
function ppom_file_get_name($file_name, $product_id) {
	
	$new_file_name = "{$product_id}-{$file_name}";
	return apply_filters('ppom_file_name_prefix', $new_file_name, $file_name, $product_id);
}