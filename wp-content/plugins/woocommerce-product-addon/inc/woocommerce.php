...
function ppom_woocommerce_show_fields() {
    ...
    echo '<div id="ppom-box-' . PPOM() -> productmeta_id . '" class="ppom-wrapper">';
...
function ppom_check_validation($product_id, $post_data, $passed=true) {
	...
	$ppom_posted_fields = $post_data['ppom']['fields'];
	$ppom_meta_fields = json_decode ( $ppom_settings->the_meta, true );
		
	foreach($ppom_meta_fields as $field) {
		
		// ppom_pa($field);
		if( empty($field['data_name']) || empty($field['required'])) continue;
...
function ppom_woocommerce_update_cart_fees($cart_items, $values) {
	...
	// If quantities is used then only qty is passed but we need to get
	// total quantity ordered
	if( $ppom_total_quantities > 0 ) {
		$ppom_item_order_qty = $ppom_total_quantities;
	}
	
	// Check if price is set by matrix
	$matrix_found = '';
	$quantities_field = ppom_has_field_by_type($wc_product->get_id(), 'pricematrix');
	if( isset($quantities_field[0]) ) {
		
		$quantities_field = $quantities_field[0];
		$matrix_found = ppom_extract_matrix_by_quantity($quantities_field, $wc_product, $ppom_item_order_qty);
		
		// ppom_pa($matrix_found);
		// Check that it's not a discount matrix
		if( $quantities_field['discount'] == '' ) {
			...
			if( !empty($matrix_found['percent']) ) {
				...
				$total_with_options	= $ppom_item_org_price + $total_option_price;
	...
	// Check if price is set by fixedprice addon
	/*$fixedprice_field = ppom_has_field_by_type($wc_product->get_id(), 'fixedprice');
	if( isset($fixedprice_field[0]) ) {
		
		$fixedprice_field = $fixedprice_field[0];
		$values['ppom']['fields']
		$options = $fixedprice_field['options'];
		$unit_price = $option['price']/$option['option'];
		ppom_pa($values);
		ppom_pa($fixedprice_field);
	}*/
	...
	} elseif($ppom_quantities_price > 0 && empty($matrix_found)) {
		$ppom_item_org_price = $ppom_quantities_price;
		$ppom_item_order_qty = 1;
	}
	...
	$cart_line_total = ($ppom_item_org_price + $total_option_price - $ppom_total_discount);
...
function ppom_woocommerce_add_item_meta($item_meta, $cart_item) {
	...
	foreach( $ppom_meta as $key => $meta ) {
		...
		if( isset( $meta['name']) ) {
			...
			$item_meta[] = array('name'	=> $meta['name'], 'value' => $meta['value'], 'hidden' => $hidden);
		} else {
...
function ppom_hide_variation_rpice_html($show, $parent, $variation) {
		...
		foreach( $cart_item['ppom']['fields'] as $key => $values) {
			...
			if( $field_type == 'file' ||  $field_type == 'cropper') {
				...
				foreach($values as $file_id => $file_data) {
					...
					$new_filename	= ppom_file_get_name($file_name, $product_id);
