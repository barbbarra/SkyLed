...
function ppom_has_category_meta( $product_id ) {
	...
	$meta_found = false;
	 if($p_categories){
		...
		$qry .= " WHERE productmeta_categories IS NOT NULL";
...
function ppom_convert_options_to_key_val($options, $meta, $product) {
	...
	foreach($options as $option) {
		...
		if( isset($option['option']) ) {
			...
			$ppom_new_option[$option['option']] = array('label'	=> apply_filters('ppom_option_label', $option_label, $option, $meta, $product), 
														...
														'option_id' => sanitize_key($data_name."-".$option['option']));
														
			if( $discount ) {
				$ppom_new_option[$option['option']]['discount'] = $discount_type;
			}
