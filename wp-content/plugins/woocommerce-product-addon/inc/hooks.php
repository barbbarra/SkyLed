    ...
    foreach($ppom_meta_fields as $field){
		...
		switch( $type ) {
    	    ...
    	    case 'cropper':
		        ...
		        // Croppie options
				$croppie_options	= ppom_get_croppie_options($field);
				// ppom_pa($croppie_options);
...
function ppom_hooks_checkbox_valided($has_value, $posted_fields, $field) {
	...
	if ( ! $has_value ) return $has_value;
	
	if ( $field['type'] != 'checkbox' ) return $has_value;
	
	// if Min/Max cheked is not defined
	/*if( ! isset($field['max_checked']) && ! isset($field['min_checked']) ) 
		return $has_value;*/
		
	$data_name = $field['data_name'];
	
	if( is_array($posted_fields[$data_name]) ) {
		
		$cb_values_count = count( $posted_fields[$data_name] );
		
		if ( !empty($field['min_checked']) && $cb_values_count < intval($field['min_checked']) ) {
			$has_value = false;
		}
		
		if ( !empty($field['max_checked']) && $cb_values_count > intval($field['max_checked']) ) {
			$has_value = false;
		}
	}
...
function ppom_hooks_render_shortcode( $atts ) {
	...
	echo '<div id="ppom-box-' . PPOM() -> productmeta_id . '" class="ppom-wrapper woocommerce">';
