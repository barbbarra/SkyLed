...
// 			console.log(canvas.toDataURL());
    ...
    $.each(ppom_file_vars.file_inputs, function(index, file_input){
    	...
    	// delete file
// 		console.log(file_id);
    	$("#ppom-file-container-"+file_id).on('click','.u_i_c_tools_del', function(e){
    		e.preventDefault();
    		...
    		if(a){
    			...
    			var filename  = $('input:checkbox[name="ppom[fields]['+file_id+']['+fileid+'][org]"]').val();
    			...
    			$.post(ppom_file_vars.ajaxurl, data, function(resp){
    				...
    				$('input:checkbox[name="ppom[fields]['+file_id+']['+fileid+'][org]"]').remove();
    	...
    	upload_instance[file_id] = new plupload.Uploader({
    		...
    		container			: 'ppom-file-container-'+file_id, // ... or DOM Element itself
    		drop_element		: 'ppom-file-container-'+file_id,
    		...
    		init: {
    			...
    			FilesAdded: function(up, files) {
    				...
    				if((file_count[file_id] + files_added) > upload_instance[file_id].settings.max_file_count){
    					alert(upload_instance[file_id].settings.max_file_count + ppom_file_vars.mesage_max_files_limit);
    				}else{
    			...
    			FileUploaded: function(up, file, info){
    				...
    				if(ext == 'png' || ext == 'gif' || ext == 'jpg' || ext == 'jpeg'){
    					...
    					$filelist_DIV[file_id].find('#u_i_c_' + file.id)
    					                       .find('.u_i_c_thumb')
    				...
    				var file_container = $filelist_DIV[file_id].find('.u_i_c_box');
    				var fileCheck = $('<input checked="checked" name="ppom[fields]['+file_id+']['+file.id+'][org]" type="checkbox"/>')
    				...
    				$filelist_DIV[file_id].find('.progress').remove();
    			},
    
    			UploadProgress: function(up, file) {
    				
    				$filelist_DIV[file_id].find('.progress-bar').css('width', file.percent + '%');
...
function ppom_show_cropped_preview( file_name, image_url, image_id ) {
	...
	croppie_options.url = image_url;
	
	$filelist_DIV[file_name]['croppie'].croppie(croppie_options);
}
